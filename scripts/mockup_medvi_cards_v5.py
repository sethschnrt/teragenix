#!/usr/bin/env python3
from pathlib import Path
from PIL import Image, ImageDraw

items = [
    ('public/images/generated/hero-cards-v4-cutout/woman-27-mint.png', 'Fat Loss', (241,248,242), 230),
    ('public/images/generated/hero-cards-v4-cutout/man-32-blue.png', 'Recovery', (244,247,250), 220),
    ('public/images/generated/hero-cards-v6-cutout/woman-44-blonde-cream.png', 'Longevity', (251,246,240), 225),
    ('public/images/generated/hero-cards-v4-cutout/man-49-sage.png', 'Vitality', (240,246,242), 225),
]
OUT = Path('/tmp/medvi-card-mockup-v5.png')
CARD_W, CARD_H = 240, 290
PASTEL_H = 180
WHITE_GAP = 12
LABEL_H = 54
R = 22
OUTER_PAD = 18
GAP = 18
canvas = Image.new('RGB', (OUTER_PAD*2 + CARD_W*4 + GAP*3, 360), (255,255,255))
d = ImageDraw.Draw(canvas)
d.rounded_rectangle((10,20,canvas.width-10,340), radius=26, fill=(255,255,255))
for i, (path, label, pastel, target_h) in enumerate(items):
    x = OUTER_PAD + i*(CARD_W+GAP)
    y = 35
    d.rounded_rectangle((x,y,x+CARD_W,y+CARD_H), radius=R, fill=(255,255,255))
    d.rounded_rectangle((x,y+14,x+CARD_W,y+14+PASTEL_H), radius=R, fill=pastel)
    d.rectangle((x,y+54,x+CARD_W,y+14+PASTEL_H), fill=pastel)
    label_y = y+14+PASTEL_H+WHITE_GAP
    d.rounded_rectangle((x+12,label_y,x+CARD_W-12,label_y+LABEL_H), radius=18, fill=(242,243,243))
    d.text((x+28,label_y+18), label, fill=(36,34,32))
    ax = x+CARD_W-40; ay = label_y+27
    d.line((ax-10,ay,ax+8,ay), fill=(36,34,32), width=2)
    d.line((ax+2,ay-6,ax+8,ay), fill=(36,34,32), width=2)
    d.line((ax+2,ay+6,ax+8,ay), fill=(36,34,32), width=2)
    im = Image.open(path).convert('RGBA')
    ratio = target_h / im.height
    im = im.resize((int(im.width*ratio), target_h))
    px = x + (CARD_W-im.width)//2
    py = y - 6
    canvas.paste(im, (px,py), im)
canvas.save(OUT)
print(OUT)
