...
function TrustItem({ label, icon }: (typeof tickerItems)[number]) {
  return (
    <div className="flex shrink-0 items-center gap-3 px-5 text-[11px] font-semibold tracking-[0.18em] text-[#0d262d] uppercase lg:px-6 lg:text-[10px]">
      <span className="flex h-6 w-6 items-center justify-center rounded-full bg-[#eef4fc] text-[#3b6ed6] ring-1 ring-[#d9e6f7]">
        {icon}
      </span>
      <span>{label}</span>
    </div>
...
export function TrustBar() {
  return (
    <section className="relative overflow-hidden border-y border-[#e3e8ef] bg-white py-4 lg:py-3">
